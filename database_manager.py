...
class DatabaseManager:
    ...
    def get_all_pracownicy(self):
        ...
        try:
            cursor.execute(
                "SELECT p.* FROM pracownik p JOIN stanowisko s ON p.id_stanowiska = s.id_stanowiska WHERE s.kadrowy = 0")
    ...
    def get_name_and_surname(self, user_id: int) -> Tuple[str, str]:
        connection, cursor = self.connect_to_db()
        try:
            query = "SELECT imie, nazwisko FROM pracownik WHERE id_pracownika = :user_id"
    ...
    def get_all_bonuses(self) -> List[Tuple[str, str]]:
        ...
        cursor.close()
        connection.close()
        return utilities.make_pretty_car_strings(cars) + utilities.make_pretty_electronical_devices_strings(
            electronical) + utilities.make_pretty_other_benefits_strings(other)
    ...
    def get_all_applications(self, id) -> List[Tuple[str, str]]:
        ...
        cursor.close()
        connection.close()
        return utilities.make_pretty_bonuses_application_strings(
            bonuses) + utilities.make_pretty_holiday_application_strings(
            holiday) + utilities.make_pretty_other_application_strings(other)
